...
{
    public class UnitTestStorkItmeGroupServ
    {
        private readonly SetDataBaseUp _setDataBaseUp;

        public UnitTestStorkItmeGroupServ()
        {
            _setDataBaseUp = new SetDataBaseUp("StorkItmeGroupServ");
        }

        private StorkItmeGroupServ CreateService(DataContext context)
            => new StorkItmeGroupServ(
                new LoggerFactory().CreateLogger<StorkItmeGroupServ>(),
                context,
        ...
        public async Task GetById_ReturnsCorrectEntity()
        {
            ...
            var expected = await context.StorkItmeGroup.FirstAsync();
        ...
        public async Task GetByUuid_ReturnsCorrectEntity()
        {
            ...
            var entity = await context.StorkItmeGroup.FirstAsync();
        ...
        public async Task GetAll_ReturnsAllEntities()
        {
            ...
            var expectedCount = await context.StorkItmeGroup.CountAsync();
        ...
        public async Task Create_IncreasesCount()
        {
            ...
            var before = await context.StorkItmeGroup.CountAsync();

            service.Create(new StorkItmeGroup
            {
                Name = "created group",
                Description = "desc"
            });

            var after = await context.StorkItmeGroup.CountAsync();
        ...
        public async Task Update_ChangesEntity()
        {
            ...
            var updated = await context.StorkItmeGroup.FirstAsync(x => x.Id == entity.Id);
        ...
        public async Task UpdateWithoutSave_DoesNotPersistUntilSaved()
        {
            ...
            var beforeSave = await context.StorkItmeGroup.AsNoTracking().FirstAsync(x => x.Id == entity.Id);
            Assert.Equal(original, beforeSave.Name);

            await context.SaveChangesAsync();

            var afterSave = await context.StorkItmeGroup.AsNoTracking().FirstAsync(x => x.Id == entity.Id);
        ...
        public async Task DeleteWithoutSave_RemovesAfterSave()
        {
            ...
            Assert.True(result);
            Assert.Equal(before, context.StorkItmeGroup.Count());
