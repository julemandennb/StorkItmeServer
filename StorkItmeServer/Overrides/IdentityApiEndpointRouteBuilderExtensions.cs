...
namespace StorkItmeServer.Overrides
{
    public static class IdentityApiEndpointRouteBuilderExtensions
    {
        ...
        public static IEndpointConventionBuilder MapIdentityApiFilterable<TUser, TRole>(this IEndpointRouteBuilder endpoints,
            ...
            where TRole : class, new()
        {
            ...
            if (!configureOptions.ExcludeRegisterPost)
            {
                ...
                routeGroup.MapPost("/register", async Task<Results<Ok, ValidationProblem>>
                    (ClaimsPrincipal claimsPrincipal,[FromBody] RegisterRequest registration, HttpContext context, [FromServices] IServiceProvider sp) =>
                {
                    var userManager = sp.GetRequiredService<UserManager<TUser>>();
                    var roleManager = sp.GetRequiredService<RoleManager<TRole>>();

                    if (await userManager.GetUserAsync(claimsPrincipal) is not { } userLogin)
                    {
                        throw new NotSupportedException($"{nameof(MapIdentityApiFilterable)} user is to in login.");
                    }

                    var isInAdminRole = await userManager.IsInRoleAsync(userLogin, "Admin");
                    var isInManagerRole = await userManager.IsInRoleAsync(userLogin, "Manager");

                    if (!isInAdminRole && !isInManagerRole)
                    {
                        throw new NotSupportedException($"{nameof(MapIdentityApiFilterable)} you does not have right role.");
                    }
                    ...
                    await userManager.AddToRoleAsync(user, "Read");
